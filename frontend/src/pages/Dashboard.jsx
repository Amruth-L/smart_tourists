...
export default function Dashboard() {
    ...
    React.useEffect(() => {
        const fetchData = async () => {
            try {
                // Fetch places for map
                const placesRes = await api.get('/places/')
                setMarkers(placesRes.data)
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <Card className="bg-gradient-to-br from-red-50 to-pink-50 hover:shadow-lg transition-shadow">
                            ...
                            <ul className="text-sm space-y-2">
                                {markers
                                    ...
                                    .slice(0, 5)
                                    .map(h => (
                                        <li key={h.id} className="py-2 border-b border-gray-200">
                                            <strong>{h.name}</strong>
                                            <div className="text-gray-600">{h.address}</div>
                                        </li>
                                    ))}
                                {markers.filter(m => m.place_type === 'hospital').length === 0 && (
                        ...
                        <Card className="bg-gradient-to-br from-orange-50 to-yellow-50 hover:shadow-lg transition-shadow">
                            ...
                            <ul className="text-sm space-y-2">
                                {markers
                                    ...
                                    .slice(0, 5)
                                    .map(r => (
                                        <li key={r.id} className="py-2 border-b border-gray-200">
                                            <strong>{r.name}</strong>
                                            <div className="text-gray-600">{r.address}</div>
                                        </li>
                                    ))}
                                {markers.filter(m => m.place_type === 'restaurant').length === 0 && (
                                    <li className="text-gray-500">No restaurants found</li>
                                )}
                            </ul>
                        </Card>
                        ...
                        <Card className="bg-gradient-to-br from-green-50 to-teal-50 hover:shadow-lg transition-shadow">
                            ...
                            <ul className="text-sm space-y-2">
                                ...
                                {markers.filter(m => m.place_type === 'attraction').length === 0 && (
                                    <li className="text-gray-500">No attractions found</li>
                                )}
                            </ul>
                        </Card>
                    </div>
